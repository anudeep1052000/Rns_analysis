from call_self_heat import run_self_heat_classical
from integrator import classical_integration_case
import os
import numpy as np
import matplotlib
...
from integrator import integrator1,stepwise_coupled_integrator,classical_adiabatic_law
...
print("mass_pc",mass_pc[1.3])
eps_gravity = np.array(mass_pc[1.3])*np.array(mass_drho_dt[1.3])/np.array(rho_mass[1.3])**2
print("epsilon_gravity",eps_gravity)
...
print("time_list in years",mass_time_list_years[1.3])
...
for i in mass_drho_dt.keys():
    ...
    pc =  mass_pc[i]
    rho = rho_mass[i]
    #rho_update,time_list_updated,drho_dt_updated = update_rho(rho,time_list,drho_dt_i)
...
#cv = run_self_heat_classical(rho=1.5e9,T=1e8,dt=5)
#print(cv)
